...
function Details() {
    useEffect(()=>{
        ...
        document.getElementById("mainImage").style.backgroundImage = "url(" + houseSelection[0].pictures[currentPicture] +")";
    })
  ...
  */
  ...
  const nextPicture = () => {
    ...
    const name = h.host.name.split(" ").map((eq, idx) => <span key={idx}>{eq}<br/></span> );
    const tags = h.tags.map((eq, idx) => <span className='tag' key={idx}>{eq}</span>);
    return(
        <div>
            <div id="mainImage">
                <div><FontAwesomeIcon icon={faChevronLeft} className="mainImageNavigation" onClick={previousPicture}/></div>
                <div><FontAwesomeIcon icon={faChevronRight} className="mainImageNavigation" onClick={nextPicture}/></div>
            </div>
            <div className="flex-row-space-between">
                <h1>{h.title}</h1>
                <div className="flex-row-space-between hostContainer">
                    <div className='hostName'>{name}</div>
                    <div><img src={h.host.picture} alt='hôte' className='hostImage'></img></div>                    
                </div>                
            </div>
            <div>{h.location}</div>
            <div className="flex-row-space-between">
                <div className='tagContainer'>{tags}</div>
                <div><Rating rating={h.rating}></Rating></div>
            </div>
